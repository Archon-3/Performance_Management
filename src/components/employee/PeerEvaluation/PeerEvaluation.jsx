...
const PeerEvaluation = () => {
  ...
  useEffect(() => {
    const fetchFormConfigAndPeers = async () => {
      try {
        setLoading(true)
        // Simulate API call
        setTimeout(() => {
          ...
          const mockPeers = [
            { id: "ASTU-ICT-002", name: "Abebe Kebede", department: "ICT", role: "Software Engineer" },
            { id: "ASTU-ICT-003", name: "Tigist Alemu", department: "ICT", role: "Database Administrator" },
            { id: "ASTU-ICT-004", name: "Dawit Mekonnen", department: "ICT", role: "Systems Analyst" },
            { id: "ASTU-ICT-005", name: "Meron Tadesse", department: "ICT", role: "Network Engineer" },
            { id: "ASTU-ICT-006", name: "Yonas Tesfaye", department: "ICT", role: "Project Manager" }
          ]
  ...
  const handleRemovePeer = (index) => {
    ...
    // Reset active peer if necessary
  ...
  const handleSubmit = (e) => {
    ...
    // Validate all ratings for the current peer
    if (activePeerIndex === null) {
      alert("Please select a peer to evaluate.")
      setIsSubmitting(false)
      return
    }
    
    const currentPeer = selectedPeers[activePeerIndex]
    const hasMissingRatings = formConfig.sections[0].criteria.some(
      criterion => !currentPeer.ratings[criterion.id]
    )
    ...
    // Simulate API call
    setTimeout(() => {
      console.log("Peer evaluation submitted:", currentPeer)
      setSuccess(`Evaluation for ${currentPeer.name} submitted successfully!`)
      setIsSubmitting(false)
        ...
        <main className={HomePageStyles.mainContent}>
          <section className={styles.contentSection}>
            <div className={styles.headerSection}>
              ...
              <p className={styles.pageSubtitle}>Select and evaluate your peers</p>
            </div>
              ...
              <div className={styles.peerSelectionPanel}>
                ...
                <div className={styles.peerList}>
                  {availablePeers.map((peer) => (
                    <div key={peer.id} className={styles.peerCard} onClick={() => handleSelectPeer(peer)}>
                      ...
                      <div className={styles.peerInfo}>
                        <h4>{peer.name}</h4>
                        <p>{peer.role}</p>
                        <span className={styles.peerDepartment}>{peer.department}</span>
                      </div>
              ...
              <div className={styles.evaluationPanel}>
                ...
                {selectedPeers.length === 0 ? (
                  <div className={styles.emptyState}>
                    ...
                    <p>No peers selected yet. Please select peers from the list to evaluate.</p>
                  </div>
                ) : (
                  <>
                    <div className={styles.selectedPeerTabs}>
                      {selectedPeers.map((peer, index) => (
                        <div 
                          key={peer.id}
                          className={`${styles.peerTab} ${activePeerIndex === index ? styles.activeTab : ''}`}
                    ...
                    {activePeerIndex !== null && (
                      <div className={styles.evaluationForm}>
                        <div className={styles.activePeerHeader}>
                          <h4>Evaluating: {selectedPeers[activePeerIndex].name}</h4>
                          <p>{selectedPeers[activePeerIndex].role} | {selectedPeers[activePeerIndex].department}</p>
                        </div>
                                ...
                                {formConfig?.sections[0].criteria.map((criterion, index) => (
                                  <tr key={criterion.id} className={styles.tableRow}>
                                    ...
                                    <td className={styles.tableCell}>
                                      <select
                                        value={selectedPeers[activePeerIndex].ratings[criterion.id] || ""}
                                        onChange={(e) => handleRatingChange(activePeerIndex, criterion.id, e.target.value)}
